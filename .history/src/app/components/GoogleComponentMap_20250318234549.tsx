...
const GoogleComponentMap: React.FC<GoogleComponentMapProps> = ({ places }) => {
  ...
  useEffect(() => {
    ...
    // Intercept console errors to detect Places API issues
    const originalConsoleError = console.error;
    console.error = (...args) => {
      // Check if this is a Places API error
      const errorMessage = args.join(' ');
      if (
        errorMessage.includes('Places API error') || 
        errorMessage.includes('not authorized to use this service') ||
        errorMessage.includes('ApiTargetBlockedMapError')
      ) {
        console.log('Places API not available, hiding place picker');
    ...
    // Also listen for window error events (backup method)
    const errorHandler = (event: ErrorEvent) => {
      if (event.message && (
        event.message.includes('Places API error') || 
        event.message.includes('not authorized to use this service') ||
        event.message.includes('ApiTargetBlockedMapError')
      )) {
        console.log('Places API not available (window error), hiding place picker');
        setPlacesApiAvailable(false);
      }
    };
      ...
      {!placesApiAvailable && (
        <div className="absolute bottom-4 left-4 bg-yellow-50 border border-yellow-200 p-3 rounded-lg shadow-md z-[1000] max-w-sm">
          <div className="flex items-start">
            ...
            <div>
              <p className="text-sm font-medium text-gray-700">Google Places API not enabled</p>
              <p className="text-xs text-gray-500 mt-1">To enable search functionality, activate the Places API in your Google Cloud Console for this API key.</p>
            </div>
            <button 
